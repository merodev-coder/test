  ...
  return (
    ...
    >
      ...
      <div className="flex items-end justify-between mb-10 reveal">
        <div>
          <p className="section-label mb-2">هاردوير مختار</p>
          <h2 className="text-3xl md:text-4xl font-black text-text-primary text-text-primary font-arabic">
            الكمبيوترات <span className="text-gradient-primary">الأكثر مبيعاً</span>
          </h2>
        </div>
        <Link
          href="/products?cat=hardware"
          className="btn-primary px-5 py-2.5 text-sm font-black flex items-center gap-2"
        >
          <span>عرض الكل</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-7 reveal">
          <Link href="/products" className="product-card group block">
            <div className="relative pt-6 px-5 border-b border-white/5 pb-4">
            ...
            <div className="p-5">
              <h3 className="text-xl font-black text-text-primary text-text-primary mb-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <button className="btn-primary px-6 py-3 text-sm font-bold flex items-center gap-2 relative z-10">
                  <span className="relative z-10">أضف للسلة</span>
                  <Icon name="ShoppingCartIcon" size={16} className="text-bg-deep relative z-10" />
                </button>
              </div>
            </div>
          </Link>
        ...
        <div className="md:col-span-5 flex flex-col gap-6">
          {featuredProducts?.slice(1, 4)?.map((product, idx) => (
            <Link
              key={product?.id}
              href="/products"
              className={`product-card group flex gap-4 p-4 reveal`}
              style={{ transitionDelay: `${(idx + 1) * 100}ms` }}
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <h3 className="text-sm font-black text-text-primary text-text-primary leading-tight line-clamp-2">
                    {product?.name}
                  </h3>
                  {product?.badge && (
                ...
                <div className="flex items-center justify-between">
                  ...
                  {!product?.inStock ? (
                    <span className="text-xs text-red-400 font-bold">نفد المخزون</span>
                  ) : (
                    <button className="text-xs btn-primary px-3 py-1.5 font-bold relative z-10">
